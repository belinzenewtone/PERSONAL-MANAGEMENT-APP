...
export async function commitImport(
  ...
  const rows = await Promise.all(items.map(async (p) => {
    const learnedCategory =
      await merchantLearningService.inferCategory(userId, p.counterparty) ??
      await merchantLearningService.inferCategory(userId, p.description);

    return {
      user_id: userId,
      amount: p.amount,
      type: p.transaction_type,
      category: learnedCategory ?? p.category,
