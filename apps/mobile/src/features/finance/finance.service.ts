...
export type TransactionAnalyticsPoint = Pick<Transaction, 'amount' | 'type' | 'transaction_date'>;
export type TransactionBalancePoint = Pick<
  Transaction,
  'amount' | 'type' | 'transaction_date' | 'category' | 'description' | 'source'
>;
...
export const financeService = {
  ...
  getAnalyticsTransactions: async (userId: string, months = 4): Promise<TransactionAnalyticsPoint[]> => {
    const from = startOfMonth(new Date(new Date().getFullYear(), new Date().getMonth() - (months - 1), 1)).toISOString();
  ...
  getBalanceTransactions: async (userId: string, period: Exclude<FilterPeriod, 'all'> = 'month'): Promise<TransactionBalancePoint[]> => {
    let query = supabase
      .from('transactions')
      .select('amount,type,transaction_date,category,description,source')
      .eq('user_id', userId);
  ...
  setPinned: async (txId: string, isPinned: boolean): Promise<void> => {
    const { error } = await supabase.from('transactions').update({ is_pinned: isPinned }).eq('id', txId);
    if (error) throw error;
  },
